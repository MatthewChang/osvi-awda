...
import robosuite_mosaic.utils.macros as macros
from robosuite_mosaic.utils import XMLError
from robosuite_mosaic.utils.mjcf_utils import find_elements, sort_elements,\
...
class MujocoXML(object):
    ...
    @staticmethod
    def _get_default_classes(default, default_dic={}):
        """
        ...
        for cls in default:
            properties = {child.tag: child for child in cls if child.tag != 'default'}
            if cls.get('class') is not None:
                default_dic[cls.get("class")] = properties
                default_dic = MujocoXML._get_default_classes(cls, default_dic)
        return default_dic
