...
class DoorController:
    ...
    def get_handle_loc(self):
        handle_loc = np.array(self._env.sim.data.site_xpos[self._env.door_handle_site_id])
        return handle_loc

    def get_dir(self):
        handle_start = np.array(self._env.sim.data.site_xpos[self._env.door_handle_start_id])
        handle_end = np.array(self._env.sim.data.site_xpos[self._env.door_handle_end_id])
        dir = handle_end - handle_start
        if self._env.task_id % 2 == 0:
            return dir
        else:
            return dir * (-1)
...
def get_expert_trajectory(env_type, controller_type, renderer=False, camera_obs=True, task=None, ret_env=False,
    ...
    success = False
    if task is not None:
        assert 0 <= task <= 3, "task should be in [0, 3]"
        task_id = task
    ...
    while True:
        try:
            ...
            pass
    while not success:
        controller = DoorController(env.env, tries=tries, ranges=action_ranges)
...
if __name__ == '__main__':
    ...
    for i in range(4):
        traj = get_expert_trajectory('SawyerDoor', config, renderer=True, camera_obs=False, task=i,
