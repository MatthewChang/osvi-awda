...
class ButtonPressController:
    ...
    def get_button_loc(self):
        button_loc = np.array(self._env.sim.data.site_xpos[self._env.target_button_id])
        return button_loc
    ...
    def reset(self):
        ...
        if "Sawyer" in self._env.robot_names:
            ...
            #self._base_rot = np.array([[1, 0, 0.], [0, -1, 0.], [0., 0., -1.]])
            self._base_rot = np.array([[0, 1, 0.], [1, 0, 0.], [0., 0., -1.]])
            #self._base_rot = np.array([[0, 1, 0], [0, 0, -1], [-1, 0, 0]])
            self._base_quat = Quaternion(matrix=self._base_rot)
        else:
    ...
    def act(self, obs):
        ...
        if self._t == 0:
            ...
            y = -(self.get_button_loc()[1] - obs[self._obs_name][1])
            x = self.get_button_loc()[0] - obs[self._obs_name][0]
        ...
        if self._start_reach < 0 and self._t < 15:
            ...
            eef_pose = self._get_target_pose(
                self.get_button_loc() - obs[self._obs_name] + [0, 0, self._hover_delta],
                obs['eef_pos'], quat_t)
...
def get_expert_trajectory(env_type, controller_type, renderer=False, camera_obs=True, task=None, ret_env=False,
    ...
    while True:
        try:
            ...
            pass
    while not success:
        controller = ButtonPressController(env.env, tries=tries, ranges=action_ranges)
...
if __name__ == '__main__':
    ...
    traj = get_expert_trajectory('PandaButton', config, renderer=True, camera_obs=False, task=4,
