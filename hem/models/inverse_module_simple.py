...
class GaussianNetwork(nn.Module):
    ...
    def forward(self,x):
        ...
        means = x[...,:dim//2]
        stds = x[...,dim//2:]
        noise = torch.normal(0,1,size=stds.shape).cuda()
...
class InverseImitation(nn.Module):
    def __init__(self, latent_dim, lstm_config, sdim=9, adim=8, n_mixtures=3, concat_state=True, const_var=False, pred_point=False, vis=dict(), transformer_feat=True,simple_policy=False):
        super().__init__()
        ...
        self.simple_policy=simple_policy
        ...
        elif self.simple_policy:
            self._action_module = nn.Sequential(
                nn.Linear(int(latent_dim + latent_dim + float(concat_state) * sdim), 1024), nn.ReLU(), nn.Linear(1024, 512), nn.ReLU(), nn.Linear(512, adim*2),GaussianNetwork()
            )
        else:
